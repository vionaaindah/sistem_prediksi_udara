...
import pandas as pd # data processing, CSV file I/O (e.g. pd.read_csv), data manipulation seperti SQL
...
n_train_time = len(values) * 0.75 # jumlah % data
...
model = Sequential()
model.add(GRU(20, input_shape=(train_X.shape[1], train_X.shape[2]), return_sequences=True)) # menggunakan 20 neurons GRU untuk layer pertama
...
model.compile(loss='mean_squared_error', optimizer='adam') # Menggunakan MSE sebagai loss function dan Adam sebagai optimizer
